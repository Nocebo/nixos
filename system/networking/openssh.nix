{
  ...
  config = lib.mkIf config.openssh.enable {
    services.openssh = {
      ...
      # Port configuration
      ports =
        if config.openssh.mode == "hardened"
        then [9922]
        else [22];
    ...
    networking.firewall.allowedTCPPorts =
      if config.openssh.mode == "soft"
      then [22]
      else if config.openssh.mode == "hardened"
      then [] # Commented out in original
      else [];

    # Nix settings for both modes, merged to avoid duplicate attrname error
    nix.settings = lib.mkMerge [
      (lib.mkIf (config.openssh.mode == "soft") {
        trusted-users = ["root" "kleanzy"];
        require-sigs = false;
      })
      (lib.mkIf (config.openssh.mode == "hardened") {
        trusted-users = ["root" "kleanzy"];
        require-sigs = false;
      })
    ];
